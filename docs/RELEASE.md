...
4. **UI polish** — improve first-run flow, game-folder selection, hierarchy, spacing, wording, status/error presentation, Apply/Restore affordances, iconography, window behavior, and version/about information. Validate the packaged UI rather than only a development checkout. **The consolidated UI revision is implemented; a maintainer-approved custom icon and final packaged visual approval remain required.**
...
Both packaged formats previously passed the functional validation-flow checks on physical Bazzite, including Flatpak portal access and sandbox-permission inspection. The UI and shared metadata have changed since those passes, so focused packaged smoke checks must be repeated after the final custom icon is approved and added. Functional, packaged, or flow QA does not constitute visual acceptance. UI polish remains incomplete until the maintainer explicitly approves the packaged appearance.
