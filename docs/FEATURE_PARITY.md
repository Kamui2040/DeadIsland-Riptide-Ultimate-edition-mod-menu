...
Integrated `main` contains **42 semantic non-default options, all Native-validated** against the accepted native baseline. All seven non-default forced-spawn choices are mutually exclusive, and the four formerly blocked choices are enabled in the integrated UI.

The Linux-native application/GUI layer is physically validated on Bazzite: isolated optional-GUI installation, PySide6 import, catalog accounting, offscreen construction, real on-screen visual QA, native installation validation, wheel construction, and the real confirmation-driven Apply/Restore transaction all pass. UI/catalog tests verify the 42-option UI catalog matches the semantic catalog exactly.
...
Linux packaging is physically validated on Bazzite for the integrated 42-option state. The distribution checker requires `src/dirue/forced_spawn_compat.py` in the sdist and `dirue/forced_spawn_compat.py` in the wheel; provenance-sensitive inherited payloads remain excluded. Using an isolated PyPA `build==1.5.0` frontend and a commit-derived `SOURCE_DATE_EPOCH`, two clean builds produced byte-identical artifacts:
...
The unconditional `game.ini` / `menumain_pc.xui` replacement question is closed: accepted native comparison proves it is cosmetic/branding-only and unnecessary for Linux gameplay parity. Released gameplay-option parity and bounded packaging validation are complete on the integrated fork state.

No further routine gameplay or packaging QA is required absent a newly identified risk. No release, public binary, Nexus publication, upstream submission, GitHub Actions use, or other external publication has been authorized.
