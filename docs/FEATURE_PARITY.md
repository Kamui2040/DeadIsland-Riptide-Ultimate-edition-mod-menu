...
Representative native gameplay QA validates camera FOV82, Better Firearms POV82, darker storm/night, Run with weapons, One Hit AI, supersize behavior on active infected/walker AI, Forced Suiciders, Better Firearms Upgrading, Forced Butchers, Forced Rams, Forced Bloaters, and Forced Thugs, with stable playable sessions and exact pristine restore after each bounded test. The Better Firearms Upgrading evidence uses the same fully upgraded Magnum/revolver path before and after mutation and preserves the exact pristine-to-UI multiplier implied by the accepted raw reload values. The supersize release-equivalent four-member transform does not necessarily affect every corpse-decoy/ground actor; that observed entity-state variation is retained as upstream/native behavior rather than expanded beyond released parity.

Linux packaging is physically validated on Bazzite for the integrated 38-option state. Provenance-sensitive inherited payloads are excluded from distributions; the build backend pins `setuptools==83.0.0` and normalizes sdist gzip/tar metadata to a commit-derived `SOURCE_DATE_EPOCH`; two clean builds produced byte-identical wheel and sdist artifacts; the distribution checker passed; and the built wheel installed with working CLI plus expected GUI entry-point metadata. The packaging run left the repository clean and did not mutate the pristine game archive or retained backup.

The focused 42-option branch adds `src/dirue/forced_spawn_compat.py` and fixes direct `python -m dirue.cli` execution. A subprocess regression test confirms direct module invocation now executes, but a fresh distribution build has not yet been accepted for this branch. That bounded packaging refresh is the remaining technical integration-readiness gate; it does not reopen gameplay parity.

The unconditional `game.ini` / `menumain_pc.xui` replacement question is closed: accepted native comparison proves it is cosmetic/branding-only and unnecessary for Linux gameplay parity. Released gameplay-option parity is complete on the focused branch.

No further gameplay QA is required absent a newly identified gameplay risk. After the bounded packaging refresh, integrating the focused branch into `main` still requires explicit approval. No release, public binary, Nexus publication, upstream submission, GitHub Actions use, or other external publication has been authorized.